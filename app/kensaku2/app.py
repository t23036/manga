...
app = FastAPI() #FastAPIアプリケーションのオブジェクトを作成

app.add_middleware( #CORSの設定(異なるオリジン(場所)からのアクセスを許可)
    CORSMiddleware,
...
def search_users(item: Item): #リクエストボディをItemクラスとして受け取る
    with sqlite3.connect('example.db') as conn: #SQLiteデータベースの接続(自動で閉じる)
